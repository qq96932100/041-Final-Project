...
for k in stock_number:
    ...
    # /.commit()        確認完成
    for i in range(6,13):
        df = pd.read_csv('C:\041-Final-Project\django\sample\back_end\0050\0050 2021 '+str(i)+'月'+'.csv', index_col=[0])
        # 寫入資料表 /.to_sql 關鍵參數為:資料表名字,連線參數,
        # 是否已存在,是否寫入pandas dataframe索引值
        df.to_sql('stock_'+k, conn, if_exists='append', index=False)
    for j in range(1,7):
        df = pd.read_csv('C:\041-Final-Project\django\sample\back_end\0050\0050 2022 '+str(j)+'月'+'.csv', index_col=[0])
        df.to_sql('stock_'+k, conn, if_exists='append', index=False)
